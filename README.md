...
- This README is intentionally concise for a college CS student: the code is exploratory and small enough to be read end-to-end in a few minutes. Focus on `get_game_ids()`, `get_pbp_data()`, and `get_all_player_stats_of_game()` to understand the core parsing flow.

---

If you want, I can: add a small example output section, add a `requirements.txt` (if switching to `requests`), or implement CSV export. Which would you prefer next?
